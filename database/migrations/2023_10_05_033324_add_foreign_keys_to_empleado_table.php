...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('empleado', function (Blueprint $table) {
            ...
            $table->foreign(['id_arl'], 'empresa_ibfk_5')->references(['ID_arl'])->on('arl');
            $table->foreign(['id_pens'], 'empresa_ibfk_7')->references(['ID_pens'])->on('pensiones');
            $table->foreign(['id_doc'], 'empresa_ibfk_1')->references(['ID_Doc'])->on('tipo_doc');
            $table->foreign(['id_eps'], 'empresa_ibfk_4')->references(['ID_eps'])->on('eps');
            $table->foreign(['id_ces'], 'empresa_ibfk_6')->references(['ID_ces'])->on('cesantias');
        });
    }
